...
export default function CallRoom() {
  ...
  const streamRef = useRef(null); // 🔥 NEW: Ref to hold stream for cleanup
  ...
  useEffect(() => {
    ...
    const initCall = async () => {
      try {
        ...
        streamRef.current = localStream; // 🔥 Save to ref for cleanup access
    ...
    return () => {
      ...
      // 🔥 FIX: Explicitly stop all camera/mic tracks using the Ref
      // This ensures the hardware light goes off immediately
  ...
  const cleanupAndLeave = () => {
    // Force cleanup logic is handled by useEffect return, 
    // but we navigate away to trigger it.
    navigate('/chat');
  };
