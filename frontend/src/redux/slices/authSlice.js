...
export const loginUser = createAsyncThunk(
	...
	async (credentials, thunkAPI) => {
		try {
			...
			const user = res.data.user ?? res.data?.data?.user ?? res.data; // User might be at root or in .user/.data.user
...
const authSlice = createSlice({
	...
	reducers: {
		logout: (state) => {
			...
			localStorage.removeItem("token");
			localStorage.removeItem("meId");
			state.checkingAuth = false; // Added to stop further checks on logout
		},
